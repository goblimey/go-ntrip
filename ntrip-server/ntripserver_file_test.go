...
// These tests create files in temporary directories.
...
var inputDataBytes = [...]byte{
	...
	/// Anything which is not an RTCM3 message should be ignored.
	'j', 'u', 'n', 'k',
...
// TestWaitAndConnectToInput tests waitAndConnectToInput by creating
...
func TestWaitAndConnectToInput(t *testing.T) {
	...
	filenames := make([]InputFile, 0)
	filenames = append(filenames, InputFile{"a"})
	filenames = append(filenames, InputFile{"b"})
	filenames = append(filenames, InputFile{"c"})

	// Test with file "a".
	os.Create("a")
	file := waitAndConnectToInput(filenames)
	if file.Name() != "a" {
